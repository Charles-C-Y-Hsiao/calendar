  ...
  // All UI actions can request a save close together (create, drag/resize,
  // editor close, or a refresh-triggered update).  Serialize them so an older
  // DOM snapshot cannot finish after a newer one and overwrite the new item.
  let saveChain = Promise.resolve();
  let saveRevision = 0;

  function enqueueSave(task) {
    const revision = ++saveRevision;
    const run = saveChain.catch(() => {}).then(() => task(revision));
    saveChain = run.catch(() => {});
    return run;
  }

  async function refreshWeekAfterSave() {
    const context = window.weeklyCalendarBulkContext;
    if (!context || typeof context.setupInteract !== 'function') return;
    await loadWeek(viewStart, context.setupInteract, context.getDayCols);
  }

  async function persistDayNow(arg, revision) {
    let dateStr;
    let payload;
    ...
    try {
      ...
      if (ok && revision === saveRevision) await refreshWeekAfterSave();
  ...
  function persistDay(arg) {
    return enqueueSave(revision => persistDayNow(arg, revision));
  }
  ...
  async function persistWeekNow(revision) {
  ...
  function persistWeek() {
    return enqueueSave(revision => persistWeekNow(revision));
  }
