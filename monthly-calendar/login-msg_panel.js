    ...
    // console.log({ trigger, dialog_close, overlay, dialog, input, clearBtn, loginBtn });
    console.log({ trigger, dialog_close, overlay, dialog });
    ...
    document.addEventListener("click", (e) => {
        // console.log('event: ', e)
        const trigger = e.target.closest("#show-user-name");
        // console.log("closest('#show-user-name') =", trigger);
        if (!trigger) {
            // console.log('trigger #show-user-name return')
            return;
        };
    ...
    //     if (!trigger) return;
    //     console.log("HIT show-user-name!");
    // });
    ...
    clearBtn.addEventListener("click", () => {
        ...
        counter.classList.remove("counter-yellow", "counter-green", "counter-red", "weight");
    ...
    loginBtn.addEventListener("click", () => {
        ...
        if (!isEightDigits) {
            ...
            openMsgWarning({ message: "Please enter exactly 8 digits (0-9)."});
            return;
        }
        // 原生泡泡提示
        // if (!isEightDigits) {
    ...
    async function sendNameToBackend(nameJson) {
        ...
        //     console.log('userNameSpan', user_name)
        //     userNameSpan.textContent = `Hi, ${user_name}`;
        // }
        updateUserNameUI(user_name); /*in task-list.js */
    ...
    function openMsgWarning({ message, durationMs = 3000 }) {
        ...
        overlay.innerHTML = `
            <div class="msg_panel">
                ...
                <div class="msg_panel-subtitle">${message}</div>
        ...
        function update() {
            remaining -= step;
            if (remaining <= 0) { closeWarning(); return; }
        ...
        function closeWarning() {
            clearInterval(timerId); overlay.remove();            
        }        
    ...
    input.addEventListener("input", () => {
        ...
        if (textLength > 8) {
            ...
            warningText.style.display = "inline-flex";
            warningText.classList.remove("shake-warning");
